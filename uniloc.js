(function (root) {
    ...
    LookupTree.prototype.add = function (parts, route) {
        var i, branch
        var branches = parts.map(function (part) {
            return part[0] == ':' ? ':' : part
        })
        ...
        return {
            lookup: function (uri, method) {
                ...
                for (i = 0; i != queryParts.length; i++) {
                    ...
                    var existingObjectKeyValue = options[x[0]];
                    var value = decodeURIComponent(x[1]);

                    if(existingObjectKeyValue) {
                        if(isArray(existingObjectKeyValue)) {
                            existingObjectKeyValue.push(value);
                        }
                        else {
                            options[x[0]] = [existingObjectKeyValue, value];
                        }
                    } else {
                        options[x[0]] = value;
                    }
